...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let percent = { value: 0 };
    ...
    // Animate battery fill and percentage
    gsap.to(percent, {
      value: 100,
      duration: 1.5, // total loading time
      ease: "linear",
      onUpdate: () => {
        let val = Math.round(percent.value);
        document.querySelector(".charge").style.width = val + "%";
        document.querySelector(".loading-text").textContent = `Charging... ${val}%`;
      },
      onComplete: () => {
        gsap.to(".loader", {
          opacity: 0,
